...
def simulate():
    ...
    for i, t_ in enumerate(tqdm(t[:-1])):
        ...
        t_stop = time.time()
        # print(f"{i:0>5}/{len(t):0>5}: {t_:.2f}/{t[-1]:.2f} ({t_stop-t_start:.2f} sec)")
    ...
    r_plot_inh = r_inh[:]
    return t_plot, r_plot_exc, r_plot_inh, rho_plot_exc, rho_plot_inh, v

t_plot, r_plot_exc, r_plot_inh, rho_plot_exc, rho_plot_inh, v = simulate()
#
...
ax.plot_surface(Y, X, rho_plot_exc,
...
c = ax.pcolormesh(Y, X, rho_plot_inh, cmap='viridis', vmin=z_min, vmax=z_max)
fig.colorbar(c, ax=ax)
ax.plot_surface(Y, X, rho_plot_inh,
                cmap="jet", linewidth=0, antialiased=False, rcount=100, ccount=100)
