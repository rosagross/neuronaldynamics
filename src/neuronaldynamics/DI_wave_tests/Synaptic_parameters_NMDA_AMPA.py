...
p_values = np.linspace(0.25, 0.75, 100)
n_E = len(E_values)
...
simulation_name = 'AMPDA_NDMA_diw'
...
if simulate:
    ...
    # for i in tqdm(range(p_values.shape[0])):
        parameters = {'intensity': E_val_j, 'fraction_nmda': p_val_i, 'fraction_gaba_a': 0.95, 'fraction_ex': 0.5,
                  'i_scale': 5.148136e-6, 'theta': 0, 'detrend': True, 'delay_signal': True,
                  'fn_session': fn_session, 'T': T, 'name': 'param_diw_single_run_1', 'dt': dt, 'min_delay': 0,
...
if single_shot_plot:
    ...
    ax.set_title(f'Variation of NMDA/AMPA ratio')
    ax.set_ylabel('NMDA/AMPA ratio')
    ax.set_xlabel('t (ms)')
    ax.grid(True)
    ax.set_xticks([0, 2, 4, 5, 6, 7, 8, 10, 12])
